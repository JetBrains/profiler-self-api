...
namespace JetBrains.Profiler.SelfApi.Impl
{
  internal sealed class DynamicMeasureProfilerApi
  {
    ...
    private DynamicMeasureProfilerApi(GetFeaturesDelegate getFeatures, StartCollectingDataDelegate start, StopCollectingDataDelegate stop, SaveDataDelegate saveData, DropDataDelegate dropData, DetachDelegate detach)
    {
    ...
    public static DynamicMeasureProfilerApi TryCreate()
    {
      ...
      return new DynamicMeasureProfilerApi(getFeatures, start, stop, saveData, dropData, detach);
    }
  }
}